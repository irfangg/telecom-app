...
const {sendEmail,postData} = require("../utils/sendEmail");
...
const register = async (req, res, next) => {
  try {
    ...
    const user = new User(email, bcryptPassword, name, dob, adharNumber, new Date(), assignedMobileNumber);
    

    const postedData = await postData(`${process.env.MONGO_URI}/users`, user);
    // for mongodb
...
const login = async (req, res, next) => {
  try {
    ...
    const users = await connectDB('users');
    const data = users.filter((u)=>u.email === email);
    const user = data.length > 0 ? new User(data[0].email, data[0].password) : null;
